...
//	theWnd->ResetOverhang();
	...
	if (units > 0)
		sf_fillrect((sf_curtextsetting())->back, left - 1, bottom - units,
				right - left + 1, units);
	else if (units < 0)
		sf_fillrect((sf_curtextsetting())->back, left - 1, top - 1,
				right - left + 1, units);
...
void sf_initvideo(int W, int H, int full) {
	...
	if (screen == NULL) {
		os_fatal("Couldn't set %dx%dx%d video mode: %s", reqW, reqH,
	...
	needoff = (mustlockscreen) || (screen->w != W) || (screen->h != H)
			|| (format->BitsPerPixel != 24) || (screen->pitch != 3 * W);
...
// printf("setvideo: gm %dx%d rq %dx%d(f%d) got %dx%d needoff %d\n", W,H,reqW,reqH,full,screen->w,screen->h,needoff);
	...
	if (needoff) {
		...
		off = SDL_CreateRGBSurfaceFrom(sbuffer, W, H, 32, 4 * W, 0xff, 0xff00,
				0xff0000, 0);
...
//printf("down %d\n",e->button.button);
	...
	case SDLK_KP0:
		return ZC_NUMPAD_MIN + 0;
	case SDLK_KP1:
		return ZC_NUMPAD_MIN + 1;
	case SDLK_KP2:
		return ZC_NUMPAD_MIN + 2;
	case SDLK_KP3:
		return ZC_NUMPAD_MIN + 3;
	case SDLK_KP4:
		return ZC_NUMPAD_MIN + 4;
	case SDLK_KP5:
		return ZC_NUMPAD_MIN + 5;
	case SDLK_KP6:
		return ZC_NUMPAD_MIN + 6;
	case SDLK_KP7:
		return ZC_NUMPAD_MIN + 7;
	case SDLK_KP8:
		return ZC_NUMPAD_MIN + 8;
	case SDLK_KP9:
		return ZC_NUMPAD_MIN + 9;
	case SDLK_F1:
...
static void addtoHistory(zchar *buf) {
	...
	if (histptr)
		memmove(History + n, History, histptr * sizeof(zword));
	histptr += n;
	n -= 2;
...
static void sf_quitconf() {
	if (sf_yesnooverlay(ewidth / 2, eheight / 2, "Quit: are you sure?",
			1)==ZC_RETURN) {
