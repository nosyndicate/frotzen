...
static bb_err_t bb_initialize_map(bb_map_t *map) {
	...
	for (ix = 0; ix < map->numchunks; ix++) {
		...
		case bb_ID_RIdx:
			...
			err = bb_load_chunk_by_number(map, bb_method_Memory, &chunkres, ix);
			if (err)
				return err;
			...
			if (numres) {
				...
				resources = (bb_resdesc_t *) malloc(
						numres * sizeof(bb_resdesc_t));
				ressorted = (bb_resdesc_t **) malloc(
						numres * sizeof(bb_resdesc_t *));
				...
				qsort(ressorted, numres, sizeof(bb_resdesc_t *), &sortsplot);
		...
		case bb_ID_IFhd:
			...
			{
				uint16 val;
				bb_zheader_t *head = (bb_zheader_t *) malloc(
						sizeof(bb_zheader_t));
				if (!head)
					return bb_err_Alloc;
		...
		case bb_ID_Reso:
			...
			{
				bb_resolution_t *reso = (bb_resolution_t *) malloc(
						sizeof(bb_resolution_t));
				if (!reso)
					return bb_err_Alloc;
			...
			if (len) {
				bb_aux_pict_t *aux = (bb_aux_pict_t *) malloc(
						len * sizeof(bb_aux_pict_t));
		...
		case bb_ID_Loop:
			...
			if (len) {
				bb_aux_sound_t *aux = (bb_aux_sound_t *) malloc(
						len * sizeof(bb_aux_sound_t));
...
bb_err_t bb_load_resource(bb_map_t *map, int method, bb_result_t *res,
	...
	found = bsearch(&ptr, map->ressorted, map->numresources,
			sizeof(bb_resdesc_t *), &sortsplot);
...
bb_err_t bb_count_resources(bb_map_t *map, uint32 usage, int *num, int *min,
		int *max) {
	int ix;
...
bb_err_t bb_get_palette(bb_map_t *map, bb_palette_t **res) {
	...
	if (!map->palette) {
		...
		if (chunkres.length == 1) {
			...
			bb_color_t *colors = (bb_color_t *) malloc(
					size * sizeof(bb_color_t));
			if (!colors)
				return bb_err_Alloc;
			if (size < 1 || size > 256)
				return bb_err_Format;
