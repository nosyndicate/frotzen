...
#define HOMEDIR "HOME"
	...
	do {
		...
		case 'A':
			...
			break;
		case 'b':
			u_setup.background_color = getcolor(optarg);
			...
			break;
		case 'c':
			f_setup.context_lines = atoi(optarg);
			break;
		case 'd':
		...
		case 'e':
			...
			break;
		case 'f':
			u_setup.foreground_color = getcolor(optarg);
			u_setup.force_color = 1;
			u_setup.disable_color = 0;
		...
		case 'F':
			...
			break;
		case 'h':
			u_setup.screen_height = atoi(optarg);
			break;
		case 'i':
			f_setup.ignore_errors = 1;
			break;
		case 'l':
			f_setup.left_margin = atoi(optarg);
			break;
		case 'o':
		...
		case 'q':
			...
			break;
		case 'r':
			f_setup.right_margin = atoi(optarg);
			break;
		case 's':
			u_setup.random_seed = atoi(optarg);
			break;
		case 'S':
			f_setup.script_cols = atoi(optarg);
			break;
		case 't':
			u_setup.tandy_bit = 1;
			break;
		case 'u':
			f_setup.undo_slots = atoi(optarg);
			break;
		case 'w':
			u_setup.screen_width = atoi(optarg);
			break;
		case 'x':
			f_setup.expand_abbreviations = 1;
			break;
		case 'Z':
			f_setup.err_report_mode = atoi(optarg);
	...
	if ((p != NULL)
			&& ((strcmp(p, EXT_BLORB2) == 0) || (strcmp(p, EXT_BLORB3) == 0)
	...
	u_setup.blorb_file = malloc(
			strlen(f_setup.story_path) * sizeof(char)
					+ strlen(u_setup.blorb_name) * sizeof(char) + 4);
...
void os_init_screen(void) {
	...
	if (initscr() == NULL) { /* Set up curses */
		os_fatal(
				"Unable to initialize curses. Maybe your $TERM setting is bad.");
		exit(1);
	}
...
int os_storyfile_seek(FILE * fp, long offset, int whence) {
	...
	if (u_setup.exec_in_blorb) {
		...
		case SEEK_END:
			return fseek(fp,
					blorb_res.data.startpos + blorb_res.length + offset,
					SEEK_SET);
			break;
		case SEEK_CUR:
...
FILE *pathopen(const char *name, const char *p, const char *mode,
		char *fullname) {
	FILE * fp;
	char buf[FILENAME_MAX + 1];
	char *bp, lastch;
...
int ux_init_blorb(void) {
	...
	if (strncmp(basename(f_setup.story_file), basename(u_setup.blorb_file), 55)
			== 0) {
